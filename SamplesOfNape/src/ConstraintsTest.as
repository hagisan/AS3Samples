package
{
	...
	public class ConstraintsTest extends Sprite
	{
		public function ConstraintsTest()
		{
			var lc:int = 0;
			...
			lc++;
			new SliderHUI(this, 10, 15*lc++, "boxWidth(10-50)", "boxWidth").setParms(50, 10, 32);
			new SliderHUI(this, 10, 15*lc++, "boxHeight(10-50)", "boxHeight").setParms(50, 10, 16);
			...
			lc++;
			new SliderHUI(this, 10, 15*lc++, "bulletSize(10-50)", "circleRadius").setParms(50, 10, 30);
			new SliderHUI(this, 10, 15*lc++, "bulletVelocityX(10-300)", "velocityX").setParms(300, 10, 150);
			new SliderHUI(this, 10, 15*lc++, "bulletDensity(0.1-1.0)", "densityBall").setParms(1.0, 0.1, 0.1);
			new SliderHUI(this, 10, 15*lc++, "bulletElasticity(0.1-1.0)", "elasticityBall").setParms(1.0, 0.1, 1.0);
			new SliderHUI(this, 10, 15*lc++, "bulletAngularVel(0-360)", "angularVel").setParms(360, 0, 10);
...
class ComplexBounding extends Sprite
{
	...
	private var w:int;
	private var h:int;
	private var bullet:Body;
	
	
	public function ComplexBounding()
	{
	...
	private function oneWayHandler(cb:PreCallback):PreFlag
	{
		...
		if(firstObject.userData.name == "bullet" && secondObject.userData.name == "ball")
		{
			trace("!Hit reset velocity");
			bullet.velocity.set(Vec2.get(0, -500));
		}
	...
	private function setUp():void
	{
		...
		createFloor(w, h, materialFloor);
		createRailings(w, h, 0, h-2 * Common.circleRadius - 1, materialFloor);
		
		// Create a tower of boxes.
		var cnt:int = int(w / Common.boxWidth) + 1;
		for (var i:int = 0; i < cnt; i++)
		{
			...
			box.position.setxy(((w - 50) - Common.boxWidth * (i + 0.5)), 100);
			box.space = space;
		}
		
		// Create the rolling bullet.
		bullet = new Body(BodyType.DYNAMIC);
		bullet.userData.name = "bullet";
		var bulletShape:Circle = new Circle(Common.circleRadius, Vec2.get(0,0), materialBall);
		bulletShape.filter.collisionGroup = 2;
		bullet.shapes.add(bulletShape);
		bullet.position.setxy(70, h - 100);		//初期位置
		bullet.angularVel = Common.angularVel;				//初期回転角
		bullet.space = space;
		bullet.velocity = Vec2.get(0, Common.velocityX);	//初速度
		bullet.cbTypes.add(oneWayType);
		...
		ball.shapes.add(new Polygon(Polygon.box(Common.circleRadius * 4, Common.circleRadius * 2), materialBox));
		ball.position.setxy(50, h - Common.circleRadius  - 5);		//初期位置
	...
	private function createFloor(w:int, h:int, materialFloor:Material):void
	{
	...
	private function createRailings(w:int, h:int, x:int, y:int, materialFloor:Material):void
	{
		var railing:Body = new Body(BodyType.STATIC);
		
		railing.userData.name = "railing";
		var rail0:Polygon = new Polygon(Polygon.rect(x, y, w, -1), materialFloor);
		rail0.filter.collisionMask = ~2;
		railing.shapes.add(rail0);
		var rail1:Polygon = new Polygon(Polygon.rect(x, h, w, 1), materialFloor);
		rail1.filter.collisionMask = ~2;
		railing.shapes.add(rail1);
		var rail2:Polygon = new Polygon(Polygon.rect(x, y, -1, h), materialFloor);
		rail2.filter.collisionMask = ~2;
		railing.shapes.add(rail2);
		var rail3:Polygon = new Polygon(Polygon.rect(w, y, 1, h), materialFloor);
		rail3.filter.collisionMask = ~2;
		railing.shapes.add(rail3);
		railing.space = space;
	}
